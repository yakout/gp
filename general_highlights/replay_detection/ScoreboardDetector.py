...
class ScoreboardDetector(object):
    """docstring for ScoreboardDetector."""
    captured_frames = []
    ...
    def __init__(self, captured_frames):
        super(ScoreboardDetector, self).__init__()
    ...
    def getZeroAreaSub(self):
        ...
        # x, y = np.where(sum <= constant.FRAME_SUBTRACTION_THRESHOLD)
        for i in range(self.n):
            for j in range(self.m):
    ...
    def detectScoreboard(self):
        ...
        scoreboards = [] # np.zeros((self.n, self.m, 3)) # it can also be of size(x2-x1, y2-y1) but for simplicity
        ...
        for i in range(1, len(self.captured_frames)):
            ...
            if nonZero <= constant.SCOREBOARD_DIFF_THRESHOLD*maxArea and np.random.randint(1, len(captured_frames)) <= constant.NUMBER_OF_FRAMES_FOR_SCOREBOARD_LABELING:
                scoreboards.append(self.captured_frames[i])


        # img = Image.fromarray(scoreboard, 'RGB')
        # img.show()
        #
        # temp = scoreboard
        # for x in range(x1, x2+1):
        ...
        return x1, y1, x2, y2, scoreboards # we need also a value for comparing
